...
public class WeightedGraph {
    ...
    */
    ...
    adjList = new ArrayList<>(); //initilizes ArrayList for adjacency lists for each node
    ...
    */
    public WeightedGraph findMST() {
        ...
        for (Edge edge : adjList.get(0)) { //iterate through edges adjacent to first vertex
            priorityQueue.offer(edge); //add each to the priority queue; the priority queue prioritizes edges with the lowest weights
        }
        ...
        while (mst.edges < vertices - 1 && !priorityQueue.isEmpty()) { //continue until the the maximum number of edges in an MST have been addded to the graph and the priority queue is empty
            ...
            if (visited[u] && !visited[v]) { //if one of the nodes connected by the edge is marked as visited and the other is not
